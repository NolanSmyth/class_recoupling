...
pk_delta_dat = np.zeros((len(T_recs_unique), len(kk)))
for i, T in enumerate(T_recs_unique):
    pk_delta_dat[i, :] = pks_dd[np.where(T_recs_unique == T)[0][0]]
...
pickle.dump(pks_delta_interp, open("interps/pks_delta_interp.p", "wb"))
