 ...
 * -# thermodynamics_free() at the end, when no more calls to
 ...
 * @param pth        Input: pointer to the thermodynamics structure (containing
 * pre-computed table)
 ...
 * @param pvecback   Input: vector of background quantities (used only in case
 * z>z_initial for getting ddkappa and dddkappa; in that case, should be already
 * allocated and filled, with format short_info or larger; in other cases, will
 * be ignored)
 * @param pvecthermo Output: vector of thermodynamics quantities (assumed to be
 * already allocated)
...
double myfunc(struct thermo *pth, struct background *pba, double z) {
  ...
  double val =
      thermal_scattering_rate_mms(pba->T_idr * (1. + z), pth->dm * 1.16e13,
                                  pth->r, pth->g, pth->lam) *
      (1. + z);
  // printf("T = %f, tsr = %.16f,\n",pba->T_idr*(1.+z), val);
  if (val < 0) {
...
int thermodynamics_at_z(struct background *pba, struct thermo *pth, double z,
                        short inter_mode, int *last_index, double *pvecback,
                        double *pvecthermo) {
  ...
  if (z >= pth->z_table[pth->tt_size - 1]) {
    ...
    x0 = pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                   pth->index_th_xe];
    pvecthermo[pth->index_th_xe] = x0;

    /* Calculate dkappa/dtau (dkappa/dtau = a n_e x_e sigma_T = a^{-2}
     * n_e(today) x_e sigma_T in units of 1/Mpc) */
    pvecthermo[pth->index_th_dkappa] =
        (1. + z) * (1. + z) * pth->n_e * x0 * _sigma_ * _Mpc_over_m_;

    /* tau_d scales like (1+z)**2 */
    pvecthermo[pth->index_th_tau_d] =
        pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                  pth->index_th_tau_d] *
        pow((1 + z) / (1. + pth->z_table[pth->tt_size - 1]), 2);

    if (pth->compute_damping_scale == _TRUE_) {

      /* r_d scales like (1+z)**-3/2 */
      pvecthermo[pth->index_th_r_d] =
          pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                    pth->index_th_r_d] *
          pow((1 + z) / (1. + pth->z_table[pth->tt_size - 1]), -1.5);
    }

    /* Calculate d2kappa/dtau2 = dz/dtau d/dz[dkappa/dtau] given that
     * [dkappa/dtau] proportional to (1+z)^2 and dz/dtau = -H */
    pvecthermo[pth->index_th_ddkappa] = -pvecback[pba->index_bg_H] * 2. /
                                        (1. + z) *
                                        pvecthermo[pth->index_th_dkappa];
    ...
    pvecthermo[pth->index_th_dddkappa] =
        (pvecback[pba->index_bg_H] * pvecback[pba->index_bg_H] / (1. + z) -
         pvecback[pba->index_bg_H_prime]) *
        2. / (1. + z) * pvecthermo[pth->index_th_dkappa];
    ...
    pvecthermo[pth->index_th_wb] =
        _k_B_ / (_c_ * _c_ * _m_H_) *
        (1. + (1. / _not4_ - 1.) * pth->YHe + x0 * (1. - pth->YHe)) *
        pba->T_cmb * (1. + z);

    /* Calculate baryon adiabatic sound speed cb2 = (k_B/mu) Tb (1-1/3
     * dlnTb/dlna) = (k_B/mu) Tb (1+1/3 (1+z) dlnTb/dz) */
    /* note that m_H / mu = 1 + (m_H/m_He-1) Y_p + x_e (1-Y_p) */
    pvecthermo[pth->index_th_cb2] = pvecthermo[pth->index_th_wb] * 4. / 3.;

    /* derivatives of baryon sound speed (only computed if some non-minimal
     * tight-coupling schemes is requested) */
    if (pth->compute_cb2_derivatives == _TRUE_) {

      /* since cb2 proportional to (1+z) or 1/a, its derivative wrt conformal
       * time is given by dcb2 = - a H cb2 */
      pvecthermo[pth->index_th_dcb2] = -pvecback[pba->index_bg_H] *
                                       pvecback[pba->index_bg_a] *
                                       pvecthermo[pth->index_th_cb2];

      /* then its second derivative is given by ddcb2 = - a H' cb2 */
      pvecthermo[pth->index_th_ddcb2] = -pvecback[pba->index_bg_H_prime] *
                                        pvecback[pba->index_bg_a] *
                                        pvecthermo[pth->index_th_cb2];
    }
    ...
    if (pba->has_idm_dr == _TRUE_) {
      ...
      // NS: in original code, dmu_idm_dr = 1/2 omega_idm_dr/omega_dr /(1+z) *
      // Gamma_heat so that's what I'm going to do here as well.

      // printf("pvecthermo[pth->index_th_dmu_idm_dr]:
      // %f\n",pvecthermo[pth->index_th_dmu_idm_dr] );

      // this is (3.7) in https://arxiv.org/pdf/1907.01496.pdf. i.e. exactly
      // what shows up in perturbation equations
      // pvecthermo[pth->index_th_dmu_idm_dr] =
      // pth->a_idm_dr*pow((1.+z)/1.e7,pth->nindex_idm_dr)*pba->Omega0_idm_dr*pow(pba->h,2);
      pvecthermo[pth->index_th_dmu_idm_dr] =
          myfunc(pth, pba, z); // TODO remove ETHOS everywhere

      // Are these derivatives still correct?
      pvecthermo[pth->index_th_ddmu_idm_dr] =
          -pvecback[pba->index_bg_H] * pth->nindex_idm_dr / (1 + z) *
          pvecthermo[pth->index_th_dmu_idm_dr];
      pvecthermo[pth->index_th_dddmu_idm_dr] =
          (pvecback[pba->index_bg_H] * pvecback[pba->index_bg_H] / (1. + z) -
           pvecback[pba->index_bg_H_prime]) *
          pth->nindex_idm_dr / (1. + z) * pvecthermo[pth->index_th_dmu_idm_dr];

      /* calculate dmu_idr (self interaction) */
      pvecthermo[pth->index_th_dmu_idr] =
          pth->b_idr * pow((1. + z) / 1.e7, pth->nindex_idm_dr) *
          pba->Omega0_idr * pow(pba->h, 2);

      /* extrapolate optical depth of idm_dr and idr */
      pvecthermo[pth->index_th_tau_idm_dr] =
          pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                    pth->index_th_tau_idm_dr] +
          (pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                     pth->index_th_tau_idm_dr] -
           pth->thermodynamics_table[(pth->tt_size - 2) * pth->th_size +
                                     pth->index_th_tau_idm_dr]) *
              (z - pth->z_table[pth->tt_size - 1]) /
              (pth->z_table[pth->tt_size - 1] - pth->z_table[pth->tt_size - 2]);

      if (pvecthermo[pth->index_th_tau_idm_dr] < 0.) {
        printf("HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      }

      pvecthermo[pth->index_th_tau_idr] =
          pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
      ...
      pvecthermo[pth->index_th_g_idm_dr] =
          pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                    pth->index_th_g_idm_dr];

      /* calculate interacting dark matter sound speed */
      pvecthermo[pth->index_th_cidm_dr2] =
          4 * _k_B_ * pba->T_idr * (1. + z) / _eV_ / 3. / pth->m_idm;

      /* calculate interacting dark matter temperature (equal to idr temperature
       * at this redhsift) */
  ...
  /** - interpolate in table with array_interpolate_spline() (normal
    ...
    /* some very specific cases require linear interpolation because of a break
     * in the derivative of the functions */
    if (((pth->reio_parametrization == reio_half_tanh) &&
         (z < 2 * pth->z_reio)) ||
        ((pth->reio_parametrization == reio_inter) && (z < 50.))) {

      class_call(array_interpolate_linear(
                     pth->z_table, pth->tt_size, pth->thermodynamics_table,
                     pth->th_size, z, last_index, pvecthermo, pth->th_size,
                     pth->error_message),
    ...
    /* in the "normal" case, use spline interpolation */
      ...
      if (inter_mode == pth->inter_normal) {
        ...
        class_call(array_interpolate_spline(
                       pth->z_table, pth->tt_size, pth->thermodynamics_table,
                       pth->d2thermodynamics_dz2_table, pth->th_size, z,
                       last_index, pvecthermo, pth->th_size,
                       pth->error_message),
                   pth->error_message, pth->error_message);
      }
 ...
 * @return the error status
 */
int thermodynamics_init(struct precision *ppr, struct background *pba,
                        struct thermo *pth) {
  ...
  double z_idm_dr, z_idr, tau_idm_dr, tau_idr, Gamma_heat_idm_dr, dTdz_idm_dr,
      T_idm_dr, z, T_idr, dz, T_adia, z_adia;
  ...
  class_test((pth->YHe < _YHE_SMALL_) || (pth->YHe > _YHE_BIG_),
             pth->error_message, "Y_He=%g out of bounds (%g<Y_He<%g)", pth->YHe,
             _YHE_SMALL_, _YHE_BIG_);

  /** - check energy injection parameters */

  class_test((pth->annihilation < 0), pth->error_message,
             "annihilation parameter cannot be negative");

  class_test((pth->annihilation > 1.e-4), pth->error_message,
             "annihilation parameter suspiciously large (%e, while typical "
             "bounds are in the range of 1e-7 to 1e-6)",
             pth->annihilation);

  class_test((pth->annihilation_variation > 0), pth->error_message,
             "annihilation variation parameter must be negative (decreasing "
             "annihilation rate)");

  class_test((pth->annihilation_z < 0), pth->error_message,
             "characteristic annihilation redshift cannot be negative");

  class_test((pth->annihilation_zmin < 0), pth->error_message,
             "characteristic annihilation redshift cannot be negative");

  class_test((pth->annihilation_zmax < 0), pth->error_message,
             "characteristic annihilation redshift cannot be negative");

  class_test((pth->annihilation > 0) &&
                 ((pba->has_cdm == _FALSE_) && (pba->has_idm_dr == _FALSE_)),
             pth->error_message,
  ...
  class_test((pth->annihilation_f_halo > 0) && (pth->recombination == recfast),
             pth->error_message,
             "Switching on DM annihilation in halos requires using HyRec "
             "instead of RECFAST. Otherwise some values go beyond their range "
             "of validity in the RECFAST fits, and the thermodynamics module "
             "fails. Two solutions: add 'recombination = HyRec' to your input, "
             "or set 'annihilation_f_halo = 0.' (default).");

  class_test((pth->annihilation_f_halo < 0), pth->error_message,
             "Parameter for DM annihilation in halos cannot be negative");

  class_test((pth->annihilation_z_halo < 0), pth->error_message,
             "Parameter for DM annihilation in halos cannot be negative");
  ...
  if (pth->thermodynamics_verbose > 0)
    if ((pth->annihilation > 0) && (pth->reio_parametrization == reio_none) &&
      ...
      printf(
          "Warning: if you have DM annihilation and you use recfast with "
          "option recfast_Heswitch >= 3, then the expression for CfHe_t and "
          "dy[1] becomes undefined at late times, producing nan's. This is "
          "however masked by reionization if you are not in reio_none mode.");

  class_test((pth->decay < 0), pth->error_message,
  ...
  class_test(_not4_ == 0., pth->error_message,
             "stop to avoid division by zero");
  class_test(pth->YHe == 1., pth->error_message,
             "stop to avoid division by zero");
  ...
  /** - assign values to all indices in the structures with
   * thermodynamics_indices()*/

  class_call(thermodynamics_indices(pba, pth, preco, preio), pth->error_message,
             pth->error_message);
  ...
  class_call_except(
      ...
      pth->error_message, pth->error_message, free(pvecback));

  /** - if there is reionization, solve reionization and store values of \f$ z,
   * x_e, d \kappa / d \tau, T_b, c_b^2 \f$ with thermodynamics_reionization()*/

  if (pth->reio_parametrization != reio_none) {
    class_call_except(
        thermodynamics_reionization(ppr, pba, pth, preco, preio, pvecback),
        pth->error_message, pth->error_message,
  ...
  for (index_tau = 0; index_tau < pth->tt_size; index_tau++) {
    class_call(background_tau_of_z(pba, pth->z_table[index_tau],
                                   tau_table + index_tau),
               pba->error_message, pth->error_message);
  }
  ...
  /** - --> minus the baryon drag interaction rate, -dkappa_d/dtau = -[1/R *
   * kappa'], with R = 3 rho_b / 4 rho_gamma, stored temporarily in column
   * ddkappa */
  ...
  for (index_tau = 0; index_tau < pth->tt_size; index_tau++) {
    ...
    class_call(background_at_tau(pba, tau_table[index_tau], pba->normal_info,
                                 pba->inter_closeby, &last_index_back,
                                 pvecback),
               pba->error_message, pth->error_message);

    R = 3. / 4. * pvecback[pba->index_bg_rho_b] / pvecback[pba->index_bg_rho_g];

    pth->thermodynamics_table[index_tau * pth->th_size +
    ...
    if (pba->has_idm_dr == _TRUE_) {
      ...
      /* - --> idr interaction rate with idm_dr (i.e. idr opacity to idm_dr
       * scattering) */
      ...
      pth->thermodynamics_table[index_tau * pth->th_size +
          ...
          4. / 3. * pvecback[pba->index_bg_rho_idr] /
          pvecback[pba->index_bg_rho_idm_dr] *
          pth->thermodynamics_table[index_tau * pth->th_size +
                                    pth->index_th_dmu_idm_dr];

      /* - --> idr self-interaction rate */
      pth->thermodynamics_table[index_tau * pth->th_size +
                                pth->index_th_dmu_idr] =
          pth->b_idr *
          pow((1. + pth->z_table[index_tau]) / 1.e7, pth->nindex_idm_dr) *
          pba->Omega0_idr * pow(pba->h, 2);
    }
  ...
  /** - --> second derivative of this rate, -[1/R * kappa']'', stored
   * temporarily in column dddkappa */
  class_call(array_spline_table_line_to_line(
                 tau_table, pth->tt_size, pth->thermodynamics_table,
                 pth->th_size, pth->index_th_ddkappa, pth->index_th_dddkappa,
                 _SPLINE_EST_DERIV_, pth->error_message),
             pth->error_message, pth->error_message);
  ...
  class_call(array_integrate_spline_table_line_to_line(
                 tau_table, pth->tt_size, pth->thermodynamics_table,
                 pth->th_size, pth->index_th_ddkappa, pth->index_th_dddkappa,
                 pth->index_th_tau_d, pth->error_message),
             pth->error_message, pth->error_message);
  ...
  if (pba->has_idm_dr == _TRUE_) {
    ...
    /** --> second derivative of idm_dr interaction rate (with idr),
     * [Sinv*dmu_idm_dr]'', stored temporarily in column dddmu */
    class_call(array_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_ddmu_idm_dr,
                   pth->index_th_dddmu_idm_dr, _SPLINE_EST_DERIV_,
                   pth->error_message),
               pth->error_message, pth->error_message);

    /** - --> compute optical depth of idm, tau_idm_dr = [int_{tau_today}^{tau}
       dtau [Sinv*dmu_idm_dr] ]. This step gives -tau_idm_dr. The resulty is
       mutiplied by -1 later on. */
    class_call(array_integrate_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_ddmu_idm_dr,
                   pth->index_th_dddmu_idm_dr, pth->index_th_tau_idm_dr,
                   pth->error_message),
               pth->error_message, pth->error_message);

    /** - --> second derivative of idr interaction rate (with idm_dr),
     * [dmu_idm_idr]'', stored temporarily in column dddmu */
    class_call(array_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_dmu_idm_dr,
                   pth->index_th_dddmu_idm_dr, _SPLINE_EST_DERIV_,
                   pth->error_message),
               pth->error_message, pth->error_message);

    /** - --> compute optical depth of idr, tau_idr = [int_{tau_today}^{tau}
       dtau [dmu_idm_idr] ].
    ...
    class_call(array_integrate_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_dmu_idm_dr,
                   pth->index_th_dddmu_idm_dr, pth->index_th_tau_idr,
                   pth->error_message),
               pth->error_message, pth->error_message);
  }
  ...
  /** - --> compute damping scale:
      ...
      r_d = 2pi/k_d = 2pi * [int_{tau_ini}^{tau} dtau (1/kappa') 1/6
     (R^2+16/15(1+R))/(1+R)^2]^1/2 = 2pi * [int_{tau_ini}^{tau} dtau (1/kappa')
     1/6 (R^2/(1+R)+16/15)/(1+R)]^1/2
  ...
  if (pth->compute_damping_scale == _TRUE_) {
    ...
    class_alloc(tau_table_growing, pth->tt_size * sizeof(double),
                pth->error_message);
    ...
    for (index_tau = 0; index_tau < pth->tt_size; index_tau++) {
      ...
      class_call(background_at_tau(pba, tau_table_growing[index_tau],
                                   pba->normal_info, pba->inter_closeby,
                                   &last_index_back, pvecback),
                 pba->error_message, pth->error_message);

      R = 3. / 4. * pvecback[pba->index_bg_rho_b] /
          pvecback[pba->index_bg_rho_g];

      pth->thermodynamics_table[index_tau * pth->th_size +
                                pth->index_th_ddkappa] =
          1. / 6. /
          pth->thermodynamics_table[(pth->tt_size - 1 - index_tau) *
                                        pth->th_size +
                                    pth->index_th_dkappa] *
          (R * R / (1 + R) + 16. / 15.) / (1. + R);
    }

    /* compute second derivative of integrand, and store temporarily in column
     * "dddkappa" */
    class_call(array_spline_table_line_to_line(
                   tau_table_growing, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_ddkappa, pth->index_th_dddkappa,
                   _SPLINE_EST_DERIV_, pth->error_message),
               pth->error_message, pth->error_message);
    ...
    class_call(array_integrate_spline_table_line_to_line(
                   tau_table_growing, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_ddkappa, pth->index_th_dddkappa,
                   pth->index_th_g, pth->error_message),
               pth->error_message, pth->error_message);

    free(tau_table_growing);

    /* we could now write the result as r_d = 2pi * sqrt(integral),
       but we will first better acount for the contribution frokm the tau_ini
       boundary. Close to this boundary, R=0 and the integrand is just
       16/(15*6)/kappa' Using kappa' propto 1/a^2 and tau propro a during RD, we
       get the analytic result: int_0^{tau_ini} dtau / kappa' = tau_ini / 3 /
       kappa'_ini Thus r_d = 2pi * sqrt( 16/(15*6*3) * (tau_ini/ kappa'_ini) *
       integral) */

    tau_ini = tau_table[pth->tt_size - 1];
    dkappa_ini = pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                           pth->index_th_dkappa];
  ...
  /** - --> second derivative with respect to tau of dkappa (in view of spline
   * interpolation) */
  class_call(array_spline_table_line_to_line(
                 tau_table, pth->tt_size, pth->thermodynamics_table,
                 pth->th_size, pth->index_th_dkappa, pth->index_th_dddkappa,
                 _SPLINE_EST_DERIV_, pth->error_message),
             pth->error_message, pth->error_message);

  /** - --> first derivative with respect to tau of dkappa (using spline
   * interpolation) */
  class_call(array_derive_spline_table_line_to_line(
                 tau_table, pth->tt_size, pth->thermodynamics_table,
                 pth->th_size, pth->index_th_dkappa, pth->index_th_dddkappa,
                 pth->index_th_ddkappa, pth->error_message),
             pth->error_message, pth->error_message);

  /** - --> compute -kappa = [int_{tau_today}^{tau} dtau dkappa/dtau], store
   * temporarily in column "g" */
  class_call(array_integrate_spline_table_line_to_line(
                 tau_table, pth->tt_size, pth->thermodynamics_table,
                 pth->th_size, pth->index_th_dkappa, pth->index_th_dddkappa,
                 pth->index_th_g, pth->error_message),
  ...
  if (pth->compute_cb2_derivatives == _TRUE_) {
    ...
    class_call(array_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_cb2, pth->index_th_ddcb2,
                   _SPLINE_EST_DERIV_, pth->error_message),
               pth->error_message, pth->error_message);

    /** - ---> first derivative with respect to tau of cb2 (using spline
     * interpolation) */
    class_call(array_derive_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_cb2, pth->index_th_ddcb2,
                   pth->index_th_dcb2, pth->error_message),
               pth->error_message, pth->error_message);
  }
  ...
  for (index_tau = pth->tt_size - 1; index_tau >= 0; index_tau--) {
    ...
    /** - ---> compute g */
    g = pth->thermodynamics_table[index_tau * pth->th_size +
                                  pth->index_th_dkappa] *
        exp(pth->thermodynamics_table[index_tau * pth->th_size +
                                      pth->index_th_g]);

    /* for some very extreme models, in the last line, the exponential of a
     * large negative number could go beyond the range covered by the "double"
     * representation numbers, and be set to zero. To avoid a division by zero
     * in the next steps, it is then better to set it to the minimum non-zero
     * double (this has no impact on observables). */
    if (g == 0.)
      g = DBL_MIN;
    ...
    /** - ---> compute g' (the plus sign of the second term is correct, see def
     * of -kappa in thermodynamics module!) */
    ...
    class_test(pth->thermodynamics_table[index_tau * pth->th_size +
                                         pth->index_th_dkappa] == 0.,
               pth->error_message, "variation rate diverges");

    pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_rate] =
        sqrt(pow(pth->thermodynamics_table[index_tau * pth->th_size +
                                           pth->index_th_dkappa],
                 2) +
             pow(pth->thermodynamics_table[index_tau * pth->th_size +
                                           pth->index_th_ddkappa] /
                     pth->thermodynamics_table[index_tau * pth->th_size +
                                               pth->index_th_dkappa],
                 2) +
             fabs(pth->thermodynamics_table[index_tau * pth->th_size +
                                            pth->index_th_dddkappa] /
                  pth->thermodynamics_table[index_tau * pth->th_size +
                                            pth->index_th_dkappa]));

    /* - ---> restore correct sign for idm_dr and idr optical depth, and
     * calculate idm_dr visibility function */
    if (pba->has_idm_dr == _TRUE_) {

      /* restore the correct sign for tau_idm_dr */
      pth->thermodynamics_table[index_tau * pth->th_size +
                                pth->index_th_tau_idm_dr] *= -1.;
      ...
      pth->thermodynamics_table[index_tau * pth->th_size +
                                pth->index_th_tau_idr] *= -1.;

      /* visibility function for idm_dr : g_idm_dr = [Sinv*dmu_idm_dr] *
       * exp(-tau_idm_dr) */
      pth->thermodynamics_table[index_tau * pth->th_size +
                                pth->index_th_g_idm_dr] =
          pth->thermodynamics_table[index_tau * pth->th_size +
  ...
  class_call(array_smooth(pth->thermodynamics_table, pth->th_size, pth->tt_size,
                          pth->index_th_rate, ppr->thermo_rate_smoothing_radius,
                          pth->error_message),
             pth->error_message, pth->error_message);

  /* - ---> fill columns for ddmu_idm_dr and dddmu_idm_dr with true values, and
   * compute idm_dr temperature and sound speed */
  if (pba->has_idm_dr == _TRUE_) {

    /** - --> second derivative with respect to tau of dmu_idm_dr (in view of
    ...
    /** - --> first derivative with respect to tau of dmu_idm_dr (using spline
     * interpolation) */
    class_call(array_derive_spline_table_line_to_line(
                   tau_table, pth->tt_size, pth->thermodynamics_table,
                   pth->th_size, pth->index_th_dmu_idm_dr,
                   pth->index_th_dddmu_idm_dr, pth->index_th_ddmu_idm_dr,
                   pth->error_message),
               pth->error_message, pth->error_message);
    ...
    class_call(background_tau_of_z(pba, z, &(tau)), pba->error_message,
               pth->error_message);

    class_call(background_at_tau(pba, tau, pba->short_info, pba->inter_normal,
                                 &last_index_back, pvecback),
               pba->error_message, pth->error_message);
    ...
    Gamma_heat_idm_dr = 2. * pba->Omega0_idr * pow(pba->h, 2) *
                        myfunc(pth, pba, z) *
                        pow((1. + z), (pth->nindex_idm_dr + 1.)) /
                        pow(1.e7, pth->nindex_idm_dr);

    // printf("%f, %f, %f, %f\n",z,Gamma_heat_idm_dr,
    // Gamma_heat_idm_dr/(pvecback[pba->index_bg_H]), myfunc(pth, pba, z));

    // printf("%f",pba->Omega0_idr);

    /* (A1) --> if Gamma is not much smaller than H, set T_idm_dr to T_idm_dr =
     * T_idr = xi*T_gamma (tight coupling solution) */
    // if(Gamma_heat_idm_dr > 1.e-3 *
    ...
    /* (A2) --> otherwise, if Gamma << H, set initial T_idm_dr to the
      ...
      T_idr = pba->T_idr * (1. + z);
      // T_idm_dr =
      // Gamma_heat_idm_dr/(pvecback[pba->index_bg_a]*pvecback[pba->index_bg_H])
      //   /(1. +
      //   Gamma_heat_idm_dr/(pvecback[pba->index_bg_a]*pvecback[pba->index_bg_H]))*T_idr;
      T_idm_dr = Gamma_heat_idm_dr / (pvecback[pba->index_bg_H]) /
                 (1. + Gamma_heat_idm_dr / (pvecback[pba->index_bg_H])) * T_idr;
      dTdz_idm_dr = 2. * T_idm_dr - Gamma_heat_idm_dr /
    ...
    pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                              pth->index_th_Tidm_dr] = T_idm_dr;
    // This is the sound speed
    pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                              pth->index_th_cidm_dr2] =
        _k_B_ * T_idm_dr / _eV_ / pth->m_idm *
        (1. + dTdz_idm_dr / 3. / T_idm_dr);
    ...
    for (index_tau = pth->tt_size - 2; index_tau >= 0; index_tau--) {
      ...
      /* (B1) --> tight-coupling solution: Gamma >> H implies
       * T_idm_dr=T_idr=xi*T_gamma */

      // if(Gamma_heat_idm_dr > 1.e3 *
      // pvecback[pba->index_bg_a]*pvecback[pba->index_bg_H]){
      if (Gamma_heat_idm_dr >
          1.e3 * pvecback[pba->index_bg_a] * pvecback[pba->index_bg_H]) {
        ...
        class_call(background_at_tau(pba, tau, pba->short_info,
                                     pba->inter_normal, &last_index_back,
                                     pvecback),
                   pba->error_message, pth->error_message);
        dTdz_idm_dr = pba->T_idr;
      }

      /* (B2) --> intermediate solution: integrate differential equation
         equation dT_idm_dr/dz = 2 a T_DM - Gamma/H (T_idr - T_idm_dr) */
      else if (Gamma_heat_idm_dr > 1.e-3 * pvecback[pba->index_bg_H]) {
        // else if (0) {
        dz = pth->z_table[index_tau + 1] - pth->z_table[index_tau];

        /* (B2a) ----> if dz << H/Gamma the equation is not too stiff and the
         * traditional forward Euler method converges */
        if (dz < pvecback[pba->index_bg_H] / Gamma_heat_idm_dr / 10.) {
          ...
          Gamma_heat_idm_dr = 2. * pba->Omega0_idr * pow(pba->h, 2) *
                              ...
                              pow(1.e7, pth->nindex_idm_dr);
          // printf("%f, %f, %f, %f\n",z,Gamma_heat_idm_dr,
          // Gamma_heat_idm_dr/(pvecback[pba->index_bg_H]), myfunc(pth, pba,
          // z));
          ...
          class_call(background_at_tau(pba, tau, pba->short_info,
                                       ...
                                       pvecback),
                     pba->error_message, pth->error_message);
          dTdz_idm_dr = 2. * pvecback[pba->index_bg_a] * T_idm_dr -
                        Gamma_heat_idm_dr / (pvecback[pba->index_bg_H]) *
                            (T_idr - T_idm_dr);
        }
        ...
        /* (B2b) ----> otherwise, the equation is too stiff and the
          ...
          N_sub_steps = (int)(dz / (pvecback[pba->index_bg_H] /
                                    Gamma_heat_idm_dr / 10.)) +
                        1;
          ...
          for (n = 0; n < N_sub_steps; n++) {
            ...
            z -= dz_sub_step;
            /* final redshift last sub-step overwritten to avoid small rounding
             * error */
            if (n == (N_sub_steps - 1))
              z = pth->z_table[index_tau];
  ...
  /** - fill tables of second derivatives with respect to z (in view of spline
   * interpolation) */

  class_call(array_spline_table_lines(pth->z_table, pth->tt_size,
  ...
  class_test(
      pth->thermodynamics_table[(index_tau + 1) * pth->th_size +
                                pth->index_th_g] >
          pth->thermodynamics_table[index_tau * pth->th_size + pth->index_th_g],
      pth->error_message,
      "found a recombination redshift greater or equal to the maximum value "
      "imposed in thermodynamics.h, z_rec_max=%g",
      _Z_REC_MAX_);
  ...
  /* approximation for maximum of g, using cubic interpolation, assuming equally
   * spaced z's */
  pth->z_rec =
      pth->z_table[index_tau + 1] +
      0.5 * (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) *
          (pth->thermodynamics_table[(index_tau)*pth->th_size +
                                     pth->index_th_g] -
           pth->thermodynamics_table[(index_tau + 2) * pth->th_size +
                                     pth->index_th_g]) /
          (pth->thermodynamics_table[(index_tau)*pth->th_size +
                                     pth->index_th_g] -
           2. * pth->thermodynamics_table[(index_tau + 1) * pth->th_size +
                                          pth->index_th_g] +
           pth->thermodynamics_table[(index_tau + 2) * pth->th_size +
                                     pth->index_th_g]);
  ...
  class_test(pth->z_rec + ppr->smallest_allowed_variation >= _Z_REC_MAX_,
             pth->error_message,
             "found a recombination redshift greater or equal to the maximum "
             "value imposed in thermodynamics.h, z_rec_max=%g",
             _Z_REC_MAX_);

  class_test(pth->z_rec - ppr->smallest_allowed_variation <= _Z_REC_MIN_,
             pth->error_message,
             "found a recombination redshift smaller or equal to the maximum "
             "value imposed in thermodynamics.h, z_rec_min=%g",
             _Z_REC_MIN_);
  ...
  class_call(background_at_tau(pba, pth->tau_rec, pba->long_info,
                               pba->inter_normal, &last_index_back, pvecback),
             pba->error_message, pth->error_message);
  ...
  if (pth->compute_damping_scale == _TRUE_) {
    ...
    pth->rd_rec = (pth->z_table[index_tau + 1] - pth->z_rec) /
                      (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) *
                      pth->thermodynamics_table[(index_tau)*pth->th_size +
                                                pth->index_th_r_d] +
                  (pth->z_rec - pth->z_table[index_tau]) /
                      (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) *
                      pth->thermodynamics_table[(index_tau + 1) * pth->th_size +
                                                pth->index_th_r_d];
  }
  ...
  if (pba->has_idr == _TRUE_) {
    ...
    if (pba->has_idm_dr == _TRUE_) {
      ...
      if (pth->nindex_idm_dr >= 2) {
        ...
        /* comment: using index_tau_max (index_tau_fs) instead of pth->tt_size-1
         * ensures that the switch is always after recombination (free
         * streaming) */
      } else {
        index_tau = 0;
      }
    ...
    /* case of idr alone without idm_dr */
      ...
      index_tau = index_tau_fs - 1;
      class_call(background_tau_of_z(pba, pth->z_table[index_tau], &tau),
                 pba->error_message, pth->error_message) tau_idm_dr_fs = tau;
      pth->tau_idr_free_streaming = tau;
    }
  }
  ...
  index_tau = 0;
  while ((pth->thermodynamics_table[(index_tau)*pth->th_size +
                                    pth->index_th_exp_m_kappa] > 1. / _E_) &&
         (index_tau < pth->tt_size))
    index_tau++;

  pth->z_star =
      pth->z_table[index_tau - 1] +
      (1. / _E_ - pth->thermodynamics_table[(index_tau - 1) * pth->th_size +
                                            pth->index_th_exp_m_kappa]) /
          (pth->thermodynamics_table[(index_tau)*pth->th_size +
                                     pth->index_th_exp_m_kappa] -
           pth->thermodynamics_table[(index_tau - 1) * pth->th_size +
  ...
  class_call(background_at_tau(pba, pth->tau_star, pba->long_info,
                               pba->inter_normal, &last_index_back, pvecback),
             pba->error_message, pth->error_message);
  ...
  if (pth->compute_damping_scale == _TRUE_) {
    ...
    pth->rd_star =
        (pth->z_table[index_tau + 1] - pth->z_star) /
            (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) *
            pth->thermodynamics_table[(index_tau)*pth->th_size +
                                      pth->index_th_r_d] +
        (pth->z_star - pth->z_table[index_tau]) /
            (pth->z_table[index_tau + 1] - pth->z_table[index_tau]) *
            pth->thermodynamics_table[(index_tau + 1) * pth->th_size +
                                      pth->index_th_r_d];
  }
  ...
  index_tau = 0;
  while ((pth->thermodynamics_table[(index_tau)*pth->th_size +
                                    pth->index_th_tau_d] < 1.) &&
         (index_tau < pth->tt_size))
    index_tau++;

  pth->z_d = pth->z_table[index_tau - 1] +
             (1. - pth->thermodynamics_table[(index_tau - 1) * pth->th_size +
                                             pth->index_th_tau_d]) /
                 (pth->thermodynamics_table[(index_tau)*pth->th_size +
                                            pth->index_th_tau_d] -
                  pth->thermodynamics_table[(index_tau - 1) * pth->th_size +
  ...
  class_call(background_at_tau(pba, pth->tau_d, pba->long_info,
                               pba->inter_normal, &last_index_back, pvecback),
             pba->error_message, pth->error_message);
  ...
  if (pba->has_idm_dr == _TRUE_) {
    ...
    if ((pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                   pth->index_th_tau_idm_dr] > 1.) &&
        (pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
                                   pth->index_th_tau_idr] > 1.)) {
      index_tau = 0;
      ...
      // printf("index_tau: %i\n",index_tau);
      // printf("pth->thermodynamics_table[(index_tau)*pth->th_size+pth->index_th_tau_idm_dr]:
      // %f\n",pth->thermodynamics_table[(index_tau)*pth->th_size+pth->index_th_tau_idm_dr]
      // ); printf("tt_size-1 %i\n",pth->tt_size-1 );

      // pth->thermodynamics_table[(index_tau)*pth->th_size+pth->index_th_tau_idm_dr]
      // is nan for index_tau>0
      while ((pth->thermodynamics_table[(index_tau)*pth->th_size +
                                        pth->index_th_tau_idm_dr] < 1.) &&
             (index_tau < pth->tt_size - 1))
        index_tau++;
      ...
      // %f\n",pth->thermodynamics_table[(index_tau)*pth->th_size+pth->index_th_tau_idm_dr]
      // );
      // printf("pth->thermodynamics_table[(index_tau-1)*pth->th_size+pth->index_th_tau_idm_dr]:
  ...
  /** - find time above which visibility falls below a given fraction of its
   * maximum */
  ...
  if (pth->thermodynamics_verbose > 0) {
    ...
    if (pba->has_idm_dr == _TRUE_) {
      if ((pth->thermodynamics_table[(pth->tt_size - 1) * pth->th_size +
        ...
        printf(" -> computation of decoupling time of idm_dr and idr skipped, "
               "because z would not be in z_table\n");
      }
    }
    ...
    if (pth->thermodynamics_verbose > 1) {
      printf(" -> free-streaming approximation can be turned on as soon as "
             "tau=%g Mpc\n",
             pth->tau_free_streaming);
    }
    if ((pba->has_idr) && (pth->thermodynamics_verbose > 1)) {
      printf(" -> dark free-streaming approximation can be turned on as soon "
             "as tau=%g Mpc\n",
             tau_idm_dr_fs);
    }
  }
...
int thermodynamics_indices(struct background *pba, struct thermo *pth,
                           struct recombination *preco,
                           struct reionization *preio) {
  ...
  if (pth->reio_parametrization == reio_bins_tanh) {
    ...
    preio->reio_num_z = pth->binned_reio_num +
                        2; /* add two values: beginning and end of reio */
  ...
  if (pth->reio_parametrization == reio_many_tanh) {
    ...
    preio->reio_num_z =
        pth->many_tanh_num + 2; /* add two values: beginning and end of reio */
...
int thermodynamics_helium_from_bbn(struct precision *ppr,
  ...
  /** - 8.6173e-11 converts from Kelvin to MeV. We randomly choose 0.1 MeV to be
   * the temperature of BBN */
  z_bbn = 0.1 / (8.6173e-11 * pba->T_cmb) - 1.0;

  class_call(background_tau_of_z(pba, z_bbn, &tau_bbn), pba->error_message,
             pth->error_message);

  class_call(background_at_tau(pba, tau_bbn, pba->long_info, pba->inter_normal,
                               &last_index, pvecback),
             pba->error_message, pth->error_message);

  Neff_bbn =
      (pvecback[pba->index_bg_Omega_r] * pvecback[pba->index_bg_rho_crit] -
       pvecback[pba->index_bg_rho_g]) /
  ...
  /** - compute Delta N_eff as defined in bbn file, i.e. \f$ \Delta N_{eff}=0\f$
   * means \f$ N_{eff}=3.046\f$ */
  DeltaNeff = Neff_bbn - 3.046;

  /* the following file is assumed to contain (apart from comments and blank
     lines):
     - the two numbers (num_omegab, num_deltaN) = number of values of BBN free
     parameters
     - three columns (omegab, deltaN, YHe) where omegab = Omega0_b h^2 and
     deltaN = Neff-3.046 by definition
  ...
  */
    ...
    if (left[0] > 39) {
      ...
      if ((num_omegab == 0) && (num_deltaN == 0)) {
        ...
        class_test(sscanf(line, "%d %d", &num_omegab, &num_deltaN) != 2,
                   pth->error_message,
                   "could not read value of parameters (num_omegab,num_deltaN) "
                   "in file %s\n",
                   ppr->sBBN_file);
        ...
        class_alloc(YHe, num_omegab * num_deltaN * sizeof(double),
                    pth->error_message);
        class_alloc(ddYHe, num_omegab * num_deltaN * sizeof(double),
                    pth->error_message);
        class_alloc(YHe_at_deltaN, num_omegab * sizeof(double),
                    pth->error_message);
        class_alloc(ddYHe_at_deltaN, num_omegab * sizeof(double),
                    pth->error_message);
        array_line = 0;
      } else {
        ...
        class_test(
            sscanf(line, "%lg %lg %lg", &(omegab[array_line % num_omegab]),
                   &(deltaN[array_line / num_omegab]), &(YHe[array_line])) != 3,
            pth->error_message,
            "could not read value of parameters (omegab,deltaN,YHe) in file "
            "%s\n",
            ppr->sBBN_file);
        array_line++;
      }
  ...
  class_call(array_spline_table_lines(deltaN, num_deltaN, YHe, num_omegab,
                                      ddYHe, _SPLINE_NATURAL_,
                                      pth->error_message),
             pth->error_message, pth->error_message);
  ...
  class_test_except(
      omega_b < omegab[0], pth->error_message, free(omegab); free(deltaN);
      free(YHe); free(ddYHe); free(YHe_at_deltaN);
      free(ddYHe_at_deltaN),
      "You have asked for an unrealistic small value omega_b = %e. The "
      "corresponding value of the primordial helium fraction cannot be found "
      "in the interpolation table. If you really want this value, you should "
      "fix YHe to a given value rather than to BBN",
      omega_b);

  class_test_except(
      omega_b > omegab[num_omegab - 1], pth->error_message, free(omegab);
      free(deltaN); free(YHe); free(ddYHe); free(YHe_at_deltaN);
      free(ddYHe_at_deltaN),
      "You have asked for an unrealistic high value omega_b = %e. The "
      "corresponding value of the primordial helium fraction cannot be found "
      "in the interpolation table. If you really want this value, you should "
      "fix YHe to a given value rather than to BBN",
      omega_b);
  ...
  class_test_except(
      DeltaNeff < deltaN[0], pth->error_message, free(omegab); free(deltaN);
      free(YHe); free(ddYHe); free(YHe_at_deltaN);
      free(ddYHe_at_deltaN),
      "You have asked for an unrealistic small value of Delta N_eff = %e. The "
      "corresponding value of the primordial helium fraction cannot be found "
      "in the interpolation table. If you really want this value, you should "
      "fix YHe to a given value rather than to BBN",
      DeltaNeff);
  ...
  class_test_except(
      DeltaNeff > deltaN[num_deltaN - 1], pth->error_message, free(omegab);
      free(deltaN); free(YHe); free(ddYHe); free(YHe_at_deltaN);
      free(ddYHe_at_deltaN),
      "You have asked for an unrealistic high value of Delta N_eff = %e. The "
      "corresponding value of the primordial helium fraction cannot be found "
      "in the interpolation table. If you really want this value, you should "
      "fix YHe to a given value rather than to BBN",
      DeltaNeff);
  ...
  class_call(array_interpolate_spline(
                 deltaN, num_deltaN, YHe, ddYHe, num_omegab, DeltaNeff,
                 &last_index, YHe_at_deltaN, num_omegab, pth->error_message),
             pth->error_message, pth->error_message);

  /** - spline in remaining dimension (along omegab) */
  class_call(array_spline_table_lines(omegab, num_omegab, YHe_at_deltaN, 1,
                                      ddYHe_at_deltaN, _SPLINE_NATURAL_,
                                      pth->error_message),
             pth->error_message, pth->error_message);

  /** - interpolate in remaining dimension (along omegab) */
  class_call(array_interpolate_spline(omegab, num_omegab, YHe_at_deltaN,
                                      ddYHe_at_deltaN, 1, omega_b, &last_index,
                                      &(pth->YHe), 1, pth->error_message),
             pth->error_message, pth->error_message);
...
int thermodynamics_onthespot_energy_injection(struct precision *ppr,
  ...
  if (z > preco->annihilation_zmax) {
    ...
    annihilation_at_z =
        preco->annihilation * exp(-preco->annihilation_variation *
                                  pow(log((preco->annihilation_z + 1.) /
                                          (preco->annihilation_zmax + 1.)),
                                      2));
    ...
    annihilation_at_z =
        preco->annihilation * exp(preco->annihilation_variation *
                                  (-pow(log((preco->annihilation_z + 1.) /
                                            (preco->annihilation_zmax + 1.)),
                                        2) +
                                   pow(log((preco->annihilation_zmin + 1.) /
                                           (preco->annihilation_zmax + 1.)),
                                       2)));
  }

  rho_cdm_today = pow(pba->H0 * _c_ / _Mpc_over_m_, 2) * 3 / 8. / _PI_ / _G_ *
                  (pba->Omega0_idm_dr + pba->Omega0_cdm) * _c_ *
                  _c_; /* energy density in J/m^3 */

  u_min = (1 + z) / (1 + preco->annihilation_z_halo);

  erfc = pow(1. + 0.278393 * u_min + 0.230389 * u_min * u_min +
                 0.000972 * u_min * u_min * u_min +
                 0.078108 * u_min * u_min * u_min * u_min,
             -4);

  *energy_rate = pow(rho_cdm_today, 2) / _c_ / _c_ * pow((1 + z), 3) *
                     (pow((1. + z), 3) * annihilation_at_z +
                      preco->annihilation_f_halo * erfc) +
                 rho_cdm_today * pow((1 + z), 3) * preco->decay;
  /* energy density rate in J/m^3/s (remember that annihilation_at_z is in
   * m^3/s/Kg and decay in s^-1) */
...
int thermodynamics_energy_injection(struct precision *ppr,
  ...
  if (preco->annihilation > 0) {
    ...
    if (preco->has_on_the_spot == _FALSE_) {
      ...
      nH0 = 3. * preco->H0 * preco->H0 * pba->Omega0_b /
            (8. * _PI_ * _G_ * _m_H_) * (1. - preco->YHe);

      /* factor = c sigma_T n_H(0) / (H(0) \sqrt(Omega_m)) (dimensionless) */
      factor = _sigma_ * nH0 / pba->H0 * _Mpc_over_m_ /
      ...
      zp = z;
      class_call(thermodynamics_onthespot_energy_injection(
                     ppr, pba, preco, zp, &onthespot, error_message),
                 error_message, error_message);
      first_integrand =
          factor * pow(1 + z, 8) / pow(1 + zp, 7.5) *
          exp(2. / 3. * factor * (pow(1 + z, 1.5) - pow(1 + zp, 1.5))) *
          onthespot; // beware: versions before 2.4.3, there were wrong
                     // exponents: 6 and 5.5 instead of 8 and 7.5
      ...
      /* uncomment these lines if you also want to compute the on-the-spot for
       * comparison */
      class_call(thermodynamics_onthespot_energy_injection(
                     ppr, pba, preco, z, &onthespot, error_message),
                 error_message, error_message);
    } else {
      class_call(thermodynamics_onthespot_energy_injection(
                     ppr, pba, preco, z, &result, error_message),
                 error_message, error_message);
    }

    /* these test lines print the energy rate rescaled by (1+z)^6 in J/m^3/s,
     * with or without the on-the-spot approximation */
    /*
 ...
 * @param preio Input: pointer to reionization structure, containing the
 * parameters of the function \f$ X_e(z) \f$
 * @param xe    Output: \f$ X_e(z) \f$
 */

int thermodynamics_reionization_function(double z, struct thermo *pth,
                                         struct reionization *preio,
                                         double *xe) {
  ...
  if ((pth->reio_parametrization == reio_camb) ||
    ...
    if (z > preio->reionization_parameters[preio->index_reio_start]) {
      ...
      *xe = preio->reionization_parameters[preio->index_reio_xe_before];
      ...
      /** - --> case z < z_reio_start: hydrogen contribution (tanh of
      ...
      argument =
          (pow((1. +
           ...
           pow((1. + z),
               preio->reionization_parameters[preio->index_reio_exponent])) /
          (preio->reionization_parameters[preio->index_reio_exponent]
           /* no possible segmentation fault: checked to be non-zero in
      ...
      /** - --> case z < z_reio_start: helium contribution (tanh of simpler
       * argument) */
      ...
      if (pth->reio_parametrization == reio_camb) {
        ...
        /* no possible segmentation fault: checked to be non-zero in
         * thermodynamics_reionization() */
        *xe += preio->reionization_parameters
                   [preio->index_helium_fullreio_fraction] *
               (tanh(argument) + 1.) / 2.;
      }
    }
  ...
  if (pth->reio_parametrization == reio_bins_tanh) {
    ...
    if (z > preio->reionization_parameters[preio->index_reio_first_z +
                                           preio->reio_num_z - 1]) {
      ...
      *xe = preio->reionization_parameters[preio->index_reio_first_xe];
      ...
      i = 0;
      while (preio->reionization_parameters[preio->index_reio_first_z + i + 1] <
             z)
        i++;
      ...
      *xe =
          preio->reionization_parameters[preio->index_reio_first_xe + i] +
          0.5 *
              (tanh((z - z_jump) / preio->reionization_parameters
                                       [preio->index_reio_step_sharpness]) +
               1.) *
              (preio->reionization_parameters[preio->index_reio_first_xe + i +
                                              1] -
               preio->reionization_parameters[preio->index_reio_first_xe + i]);
    ...
    if (z > preio->reionization_parameters[preio->index_reio_first_z +
      ...
      *xe = preio->reionization_parameters[preio->index_reio_first_xe +
      ...
      for (jump = 1; jump < preio->reio_num_z - 1; jump++) {
        ...
        center = preio->reionization_parameters[preio->index_reio_first_z +
                                                preio->reio_num_z - 1 - jump];
        // before and after are meant with respect to growing z, not growing
        // time
        before = preio->reionization_parameters[preio->index_reio_first_xe +
                                                preio->reio_num_z - 1 - jump] -
                 preio->reionization_parameters[preio->index_reio_first_xe +
                                                preio->reio_num_z - jump];
        after = 0.;
        width =
            preio->reionization_parameters[preio->index_reio_step_sharpness];
  ...
  if (pth->reio_parametrization == reio_inter) {
    ...
    if (z > preio->reionization_parameters[preio->index_reio_first_z +
      ...
      *xe = preio->reionization_parameters[preio->index_reio_first_xe +
      ...
      class_test(z < z_min, pth->error_message, "");

      class_test(z > z_max, pth->error_message, "");

      double x =
          (z - preio->reionization_parameters[preio->index_reio_first_z + i]) /
          (preio->reionization_parameters[preio->index_reio_first_z + i + 1] -
           preio->reionization_parameters[preio->index_reio_first_z + i]);
  ...
  class_test(0 == 0, pth->error_message,
...
int thermodynamics_get_xe_before_reionization(struct precision *ppr,
  ...
  class_call(array_interpolate_one_growing_closeby(
                 preco->recombination_table, preco->re_size, preco->rt_size,
                 preco->index_re_z, z, &last_index, preco->index_re_xe, xe,
                 pth->error_message),
             pth->error_message, pth->error_message);
 ...
 * must be already allocated, with format short_info or larger, but does not
 * need to be filled)
 * @return the error status
 */

int thermodynamics_reionization(struct precision *ppr, struct background *pba,
                                struct thermo *pth, struct recombination *preco,
                                struct reionization *preio, double *pvecback) {
  ...
  class_alloc(preio->reionization_parameters,
              preio->reio_num_params * sizeof(double), pth->error_message);
  ...
  if ((pth->reio_parametrization == reio_camb) ||
    ...
    /** - --> set values of these parameters, excepted those depending on the
    ...
    if (pth->reio_parametrization == reio_camb) {
      preio->reionization_parameters[preio->index_reio_xe_after] =
          ...
          pth->YHe /
              (_not4_ *
               (1. - pth->YHe)); /* xe_after_reio: H + singly ionized He (note:
                                    segmentation fault impossible, checked
                                    before that denominator is non-zero) */
    }
    if (pth->reio_parametrization == reio_half_tanh) {
      preio->reionization_parameters[preio->index_reio_xe_after] =
          1.; /* xe_after_reio: neglect He ionization */
      //+ 2*pth->YHe/(_not4_*(1.-pth->YHe));    /* xe_after_reio: H + fully
      // ionized He */
    }
    preio->reionization_parameters[preio->index_reio_exponent] =
    ...
    preio->reionization_parameters[preio->index_helium_fullreio_fraction] =
        pth->YHe /
        (_not4_ * (1. - pth->YHe)); /* helium_fullreio_fraction (note:
                                       segmentation fault impossible, checked
                                       before that denominator is non-zero) */
    ...
    class_test(preio->reionization_parameters[preio->index_reio_exponent] == 0,
               pth->error_message, "stop to avoid division by zero");

    class_test(preio->reionization_parameters[preio->index_reio_width] == 0,
               pth->error_message, "stop to avoid division by zero");

    class_test(
        preio->reionization_parameters[preio->index_helium_fullreio_width] == 0,
        pth->error_message, "stop to avoid division by zero");

    /** - --> if reionization redshift given as an input, initialize the
     * remaining values and fill reionization table*/
    ...
    if (pth->reio_z_or_tau == reio_z) {
      ...
      class_call(thermodynamics_reionization_sample(ppr, pba, pth, preco, preio,
                                                    pvecback),
                 pth->error_message, pth->error_message);
    ...
    /** - --> if reionization optical depth given as an input, find reionization
     * redshift by dichotomy and initialize the remaining values */
    ...
    if (pth->reio_z_or_tau == reio_tau) {
      ...
      z_sup = ppr->reionization_z_start_max -
              ppr->reionization_start_factor * pth->reionization_width;
      class_test(z_sup < 0., pth->error_message,
                 "parameters are such that reionization cannot take place "
                 "before today while starting after z_start_max; need to "
                 "increase z_start_max");
      ...
      class_test_except(tau_sup < pth->tau_reio, pth->error_message,
                        free(preio->reionization_parameters);
                        free(preio->reionization_table),
                        "parameters are such that reionization cannot start "
                        "after z_start_max");
      ...
      counter = 0;
      while ((tau_sup - tau_inf) >
             pth->tau_reio * ppr->reionization_optical_depth_tol) {
        ...
        free(preio->reionization_table);
        class_call(thermodynamics_reionization_sample(ppr, pba, pth, preco,
                                                      preio, pvecback),
                   pth->error_message, pth->error_message);
        ...
        counter++;
        class_test(counter > _MAX_IT_, pth->error_message,
                   "while searching for reionization_optical_depth, maximum "
  ...
  if (pth->reio_parametrization == reio_bins_tanh) {
    ...
    class_test(pth->binned_reio_num < 2, pth->error_message,
               "current implementation of binned reio requires at least two "
               "bin centers");
    ...
    for (bin = 1; bin < pth->binned_reio_num; bin++) {
      class_test(pth->binned_reio_z[bin - 1] >= pth->binned_reio_z[bin],
                 pth->error_message,
                 "value of reionization bin centers z_i expected to be passed "
                 "in growing order: %e, %e",
                 pth->binned_reio_z[bin - 1], pth->binned_reio_z[bin]);
    }
    ...
    for (bin = 1; bin < preio->reio_num_z - 1; bin++) {
      preio->reionization_parameters[preio->index_reio_first_z + bin] =
    ...
    preio->reionization_parameters[preio->index_reio_first_z +
        ...
        preio->reionization_parameters[preio->index_reio_first_z +
                                       preio->reio_num_z - 2] +
        2. * (preio->reionization_parameters[preio->index_reio_first_z +
                                             preio->reio_num_z - 2] -
              preio->reionization_parameters[preio->index_reio_first_z +
                                             preio->reio_num_z - 3]);
    ...
    class_test(
        preio->reionization_parameters[preio->index_reio_start] >
            ppr->reionization_z_start_max,
        pth->error_message,
        "starting redshift for reionization = %e, reionization_z_start_max = "
        "%e, you must change the binning or increase reionization_z_start_max",
        preio->reionization_parameters[preio->index_reio_start],
        ppr->reionization_z_start_max);
    ...
    /* 6.06.2015: changed this test to simply imposing that the first z is at
     * least zero */
    /*
      class_test(preio->reionization_parameters[preio->index_reio_first_z] < 0,
      pth->error_message,
      "final redshift for reionization = %e, you must change the binning or
      redefine the way in which the code extrapolates below the first value of
      z_i",preio->reionization_parameters[preio->index_reio_first_z]);
    */
    ...
    preio->reionization_parameters[preio->index_reio_first_xe] =
        ...
        pth->YHe /
            (_not4_ *
             (1. - pth->YHe)); /* xe_after_reio: H + singly ionized He (note:
                                  segmentation fault impossible, checked before
                                  that denominator is non-zero) */
  ...
  if (pth->reio_parametrization == reio_many_tanh) {
    ...
    class_test(pth->many_tanh_num < 1, pth->error_message,
               "current implementation of reio_many_tanh requires at least one "
               "jump center");
    ...
    for (bin = 1; bin < pth->many_tanh_num; bin++) {
      class_test(pth->many_tanh_z[bin - 1] >= pth->many_tanh_z[bin],
                 ...
                 "value of reionization bin centers z_i expected to be passed "
                 "in growing order: %e, %e",
                 pth->many_tanh_z[bin - 1], pth->many_tanh_z[bin]);
    }
    ...
    for (bin = 1; bin < preio->reio_num_z - 1; bin++) {
      ...
      // other negative number is nonsense
        ...
        class_stop(pth->error_message,
                   "Your entry for many_tanh_xe[%d] is %e, this makes no sense "
                   "(either positive or 0,-1,-2)",
    ...
    /* find smallest value of z in the array. We choose
       to define it as z_0 - ppr->reionization_start_factor*step_sharpness, but
       at least zero. */
    ...
    /* if we want to model only hydrogen reionization and neglect both helium
     * reionization */
    // preio->reionization_parameters[preio->index_reio_first_xe] = 1.;

    /* if we want to model only hydrogen + first helium reionization and neglect
     * second helium reionization */
    // preio->reionization_parameters[preio->index_reio_first_xe] = 1. +
    // pth->YHe/(_not4_*(1.-pth->YHe));

    /* if we want to model hydrogen + two helium reionization */
    // preio->reionization_parameters[preio->index_reio_first_xe] = 1.
    // + 2.*pth->YHe/(_not4_*(1.-pth->YHe));

    /* pass step sharpness parameter */
    class_test(pth->many_tanh_width <= 0, pth->error_message,
  ...
  if (pth->reio_parametrization == reio_inter) {
    ...
    class_test(pth->reio_inter_num < 1, pth->error_message,
               "current implementation of reio_inter requires at least one "
               "point (z,xe)");

    /* this parametrization requires that the first z value is zero */
    class_test(pth->reio_inter_z[0] != 0., pth->error_message,
               "For reio_inter scheme, the first value of reio_inter_z[...]  "
               "should always be zero, you passed %e",
               pth->reio_inter_z[0]);
    ...
    for (point = 1; point < pth->reio_inter_num; point++) {
      class_test(pth->reio_inter_z[point - 1] >= pth->reio_inter_z[point],
                 pth->error_message,
                 "value of reionization bin centers z_i expected to be passed "
                 "in growing order, unlike: %e, %e",
                 pth->reio_inter_z[point - 1], pth->reio_inter_z[point]);
    }
    ...
    class_test(pth->reio_inter_xe[pth->reio_inter_num - 1] != 0.,
               pth->error_message,
               "For reio_inter scheme, the last value of reio_inter_xe[...]  "
               "should always be zero, you passed %e",
    ...
    for (point = 0; point < preio->reio_num_z; point++) {
      ...
      preio->reionization_parameters[preio->index_reio_first_z + point] =
      ...
      // other negative number is nonsense
        ...
        class_stop(pth->error_message,
                   "Your entry for reio_inter_xe[%d] is %e, this makes no "
                   "sense (either positive or 0,-1,-2)",
                   point, pth->reio_inter_xe[point]);
      }

      preio->reionization_parameters[preio->index_reio_first_xe + point] =
          xe_actual;
    }
  ...
  class_test(0 == 0, pth->error_message, "value of reio_z_or_tau=%d unclear",
             pth->reio_z_or_tau);
}
...
int thermodynamics_reionization_sample(
  ...
  class_call(gt_init(&gTable), gTable.error_message, pth->error_message);

  /** - (c) first line is taken from thermodynamics table, just before
   * reionization starts */
  ...
  while (preco->recombination_table[i * preco->re_size + preco->index_re_z] <
         preio->reionization_parameters[preio->index_reio_start]) {
    i++;
    class_test(i == ppr->recfast_Nz0, pth->error_message,
               "reionization_z_start_max = %e > largest redshift in "
  ...
  /** -  --> get \f$ d \kappa / d z = (d \kappa / d \tau) * (d \tau / d z) = -
   * (d \kappa / d \tau) / H \f$ */
  ...
  reio_vector[preio->index_re_dkappadtau] =
      (1. + z) * (1. + z) * pth->n_e * xe * _sigma_ * _Mpc_over_m_;

  class_test(pvecback[pba->index_bg_H] == 0., pth->error_message,
             "stop to avoid division by zero");

  reio_vector[preio->index_re_dkappadz] =
      reio_vector[preio->index_re_dkappadtau] / pvecback[pba->index_bg_H];
  ...
  /** - --> after recombination, Tb scales like (1+z)**2. Compute constant
   * factor Tb/(1+z)**2. */
  // Tba2 = Tb/(1+z)/(1+z);

  /** - --> get baryon equation of state */
  reio_vector[preio->index_re_wb] =
      _k_B_ / (_c_ * _c_ * _m_H_) *
  ...
  class_call(gt_add(&gTable, _GT_END_, (void *)reio_vector,
                    sizeof(double) * (preio->re_size)),
             gTable.error_message, pth->error_message);

  number_of_redshifts = 1;

  /** - (d) set the maximum step value (equal to the step in thermodynamics
   * table) */
  dz_max =
   ...
   * (Tb and cb2 found later by integration). The sampling in z space is found
   * here. */
  ...
  while (z > 0.) {
    ...
    class_test(dz < ppr->smallest_allowed_variation, pth->error_message,
               "stuck in the loop for reionization sampling, as if you were "
               "trying to impose a discontinuous evolution for xe(z)");
    ...
    class_call(
        thermodynamics_reionization_function(z_next, pth, preio, &xe_next),
        pth->error_message, pth->error_message);

    class_call(background_tau_of_z(pba, z_next, &tau), pba->error_message,
               pth->error_message);
    ...
    dkappadz_next = (1. + z_next) * (1. + z_next) * pth->n_e * xe_next *
                    _sigma_ * _Mpc_over_m_ / pvecback[pba->index_bg_H];

    dkappadtau_next = (1. + z_next) * (1. + z_next) * pth->n_e * xe_next *
                      _sigma_ * _Mpc_over_m_;

    class_test((dkappadz == 0.) || (dkappadtau == 0.), pth->error_message,
               "stop to avoid division by zero");
    ...
    if (relative_variation < ppr->reionization_sampling) {
      /* accept the step: get \f$ z, X_e, d kappa / d z \f$ and store in growing
       * table */
      ...
      reio_vector[preio->index_re_dkappadtau] =
          dkappadz * pvecback[pba->index_bg_H];
      ...
      dz =
          MIN(0.9 * (ppr->reionization_sampling / relative_variation), 5.) * dz;
      dz = MIN(dz, dz_max);
    } else {
  ...
  class_alloc(preio->reionization_table,
  ...
  class_call(gt_getPtr(&gTable, (void **)&pData), gTable.error_message,
             pth->error_message);

  /** - (h) copy growTable to reionization_temporary_table (invert order of
   * lines, so that redshift is growing, like in recombination table) */
  for (i = 0; i < preio->rt_size; i++) {
    memcopy_result = memcpy(preio->reionization_table + i * preio->re_size,
  ...
  /** - (i) free the growTable with gt_free() , free vector of reionization
   * variables */
  class_call(gt_free(&gTable), gTable.error_message, pth->error_message);
  ...
  for (i = preio->rt_size - 1; i > 0; i--) {
    ...
    class_call(background_at_tau(pba, tau, pba->normal_info, pba->inter_normal,
                                 &last_index_back, pvecback),
               pba->error_message, pth->error_message);

    dz = (preio->reionization_table[i * preio->re_size + preio->index_re_z] -
          preio->reionization_table[(i - 1) * preio->re_size +
                                    preio->index_re_z]);
    ...
    dTdz =
        ...
        2. * mu / _m_e_ * 4. * pvecback[pba->index_bg_rho_g] / 3. /
            pvecback[pba->index_bg_rho_b] * opacity *
            (pba->T_cmb * (1. + z) -
             preio->reionization_table[i * preio->re_size +
                                       preio->index_re_Tb]) /
            pvecback[pba->index_bg_H];

    if (preco->annihilation > 0) {

      class_call(thermodynamics_energy_injection(
                     ppr, pba, preco, z, &energy_rate, pth->error_message),
                 pth->error_message, pth->error_message);

      // old approximation from Chen and Kamionkowski:
      // chi_heat =
      // (1.+2.*preio->reionization_table[i*preio->re_size+preio->index_re_xe])/3.;
      ...
      if (xe < 1.)
        chi_heat =
            MIN(0.996857 * (1. - pow(1. - pow(xe, 0.300134), 1.51035)), 1);
      else
        chi_heat = 1.;

      dTdz +=
          -2. / (3. * _k_B_) * energy_rate * chi_heat /
          (preco->Nnow * pow(1. + z, 3)) /
          (1. + preco->fHe +
           preio->reionization_table[i * preio->re_size + preio->index_re_xe]) /
    ...
    preio->reionization_table[(i - 1) * preio->re_size + preio->index_re_wb] =
        _k_B_ / (_c_ * _c_ * mu) *
        preio
            ->reionization_table[(i - 1) * preio->re_size + preio->index_re_Tb];
  ...
  /** - --> spline \f$ d \tau / dz \f$ with respect to z in view of integrating
   * for optical depth */
  class_call(array_spline(preio->reionization_table, preio->re_size,
                          preio->rt_size, preio->index_re_z,
                          preio->index_re_dkappadz, preio->index_re_d3kappadz3,
                          _SPLINE_EST_DERIV_, pth->error_message),
             pth->error_message, pth->error_message);

  /** - --> integrate for optical depth */
  class_call(array_integrate_all_spline(
                 preio->reionization_table, preio->re_size, preio->rt_size,
                 preio->index_re_z, preio->index_re_dkappadz,
                 preio->index_re_d3kappadz3,
                 &(preio->reionization_optical_depth), pth->error_message),
             pth->error_message, pth->error_message);
...
int thermodynamics_recombination(struct precision *ppr, struct background *pba,
                                 struct thermo *pth,
                                 struct recombination *preco,
                                 double *pvecback) {
  ...
  if (pth->recombination == hyrec) {
    ...
    class_call(
        thermodynamics_recombination_with_hyrec(ppr, pba, pth, preco, pvecback),
        pth->error_message, pth->error_message);
  }

  if (pth->recombination == recfast) {

    class_call(thermodynamics_recombination_with_recfast(ppr, pba, pth, preco,
                                                         pvecback),
               pth->error_message, pth->error_message);
  }
 ...
 * @param pvecback Input: pointer to an allocated (but empty) vector of
...
int thermodynamics_recombination_with_hyrec(struct precision *ppr,
  ...
  class_call(background_w_fld(pba, pba->a_today, &w_fld, &dw_over_da_fld,
                              &integral_fld),
             pba->error_message, pth->error_message);
  param.w0 = w_fld;
  ...
  param.fHe =
      param.Y / (1 - param.Y) / 3.97153; /* abundance of helium by number */
  param.zstart = ppr->recfast_z_initial; /* Redshift range */
  param.zend = 0.;
  param.dlna = 8.49e-5;
  param.nz = (long)floor(2 + log((1. + param.zstart) / (1. + param.zend)) /
                                 param.dlna);
  ...
  class_alloc(buffer, buf_size, pth->error_message);
  ...
  for (j = 1; j < NTM; j++) {
    rate_table.logAlpha_tab[0][j] =
        (double *)(rate_table.logAlpha_tab[0][j - 1] + NTR);
  }
  rate_table.logAlpha_tab[1][0] =
      (double *)(rate_table.logAlpha_tab[0][NTM - 1] + NTR);
  for (j = 1; j < NTM; j++) {
    rate_table.logAlpha_tab[1][j] =
        (double *)(rate_table.logAlpha_tab[1][j - 1] + NTR);
  }
  rate_table.logR2p2s_tab =
      (double *)(rate_table.logAlpha_tab[1][NTM - 1] + NTR);
  ...
  for (i = 0; i < NTR; i++)
    rate_table.logTR_tab[i] =
        log(TR_MIN) + i * (log(TR_MAX) - log(TR_MIN)) / (NTR - 1.);
  for (i = 0; i < NTM; i++)
    rate_table.TM_TR_tab[i] =
        TM_TR_MIN + i * (TM_TR_MAX - TM_TR_MIN) / (NTM - 1.);
    ...
    for (j = 0; j < NTM; j++) {
      for (l = 0; l <= 1; l++) {
        if (fscanf(fA, "%le", &(rate_table.logAlpha_tab[l][j][i])) != 1)
          class_stop(pth->error_message, "Error reading hyrec data file %s",
                     ppr->hyrec_Alpha_inf_file);
        rate_table.logAlpha_tab[l][j][i] =
            log(rate_table.logAlpha_tab[l][j][i]);
      }
    }

    if (fscanf(fR, "%le", &(rate_table.logR2p2s_tab[i])) != 1)
      class_stop(pth->error_message, "Error reading hyrec data file %s",
                 ppr->hyrec_R_inf_file);
  ...
  /** - Normalize 2s--1s differential decay rate to L2s1s (can be set by user in
   * hydrogen.h) */
  L2s1s_current = 0.;
  for (b = 0; b < NSUBLYA; b++)
  ...
  /** - Compute the recombination history by calling a function in hyrec (no
   * CLASS-like error management here) */
  ...
  /** - allocate memory for thermodynamics interpolation tables (size known in
   * advance) and fill it */

  class_alloc(preco->recombination_table,
              preco->re_size * preco->rt_size * sizeof(double),
              pth->error_message);

  for (i = 0; i < Nz; i++) {

    /** - --> get redshift, corresponding results from hyrec, and background
     * quantities */
    ...
    class_call(array_interpolate_cubic_equal(
                   -log(1. + param.zstart), param.dlna, xe_output, param.nz,
                   -log(1. + z), &xe, pth->error_message),
               pth->error_message, pth->error_message);

    class_call(array_interpolate_cubic_equal(
                   -log(1. + param.zstart), param.dlna, Tm_output, param.nz,
                   -log(1. + z), &Tm, pth->error_message),
               pth->error_message, pth->error_message);
    ...
    */
    ...
    /* Hz is H in inverse seconds (while pvecback returns [H0/c] in inverse
     * Mpcs) */
    Hz = pvecback[pba->index_bg_H] * _c_ / _Mpc_over_m_;

    /** - --> store the results in the table */

    /* results are obtained in order of decreasing z, and stored in order of
     * growing z */

    /* redshift */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_z) = z;

    /* ionization fraction */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_xe) = xe;

    /* Tb */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_Tb) = Tm;

    /* wb = (k_B/mu) Tb */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_wb) =
        _k_B_ / (_c_ * _c_ * _m_H_) *
        (1. + (1. / _not4_ - 1.) * pth->YHe + xe * (1. - pth->YHe)) * Tm;
    ...
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_cb2) =
        *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
          preco->index_re_wb) *
        (1. - rec_dTmdlna(xe, Tm, pba->T_cmb * (1. + z), Hz, param.fHe,
                          param.nH0 * pow((1 + z), 3) * 1e-6,
                          energy_injection_rate(&param, z)) /
                  Tm / 3.);

    /* dkappa/dtau = a n_e x_e sigma_T = a^{-2} n_e(today) x_e sigma_T (in units
     * of 1/Mpc) */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_dkappadtau) =
  ...
  class_stop(pth->error_message,
             "you compiled without including the HyRec code, and now wish to "
             "use it. Either set the input parameter 'recombination' to "
             "something else than 'HyRec', or recompile after setting in the "
             "Makefile the appropriate path HYREC=... ");
...
int thermodynamics_recombination_with_recfast(struct precision *ppr,
  ...
  /* contains all fixed parameters which should be passed to
  ...
  class_call(initialize_generic_integrator(_RECFAST_INTEG_SIZE_, &gi),
             gi.error_message, pth->error_message);
  ...
  /* preco->H0 is H0 in inverse seconds (while pba->H0 is [H0/c] in inverse
   * Mpcs) */
  preco->H0 = pba->H0 * _c_ / _Mpc_over_m_;
  ...
  /* H fudging */
  class_test((ppr->recfast_Hswitch != _TRUE_) &&
                 (ppr->recfast_Hswitch != _FALSE_),
             pth->error_message, "RECFAST error: unknown H fudging scheme");
  ...
  class_test((ppr->recfast_Heswitch < 0) || (ppr->recfast_Heswitch > 6),
             pth->error_message, "RECFAST error: unknown He fudging scheme");
  ...
  preco->Nnow =
      3. * preco->H0 * preco->H0 * OmegaB / (8. * _PI_ * _G_ * mu_H * _m_H_);
  pth->n_e = preco->Nnow;
  ...
  class_test(zinitial < ppr->recfast_z_He_3, pth->error_message,
             "increase zinitial, otherwise should get initial conditions from "
             "recfast's get_init routine (less precise anyway)");
  ...
  for (i = 0; i < Nz; i++) {
    ...
    /** - --> second approximation: first Helium recombination (analytic
       approximation) */
      ...
      if (z > ppr->recfast_z_He_1 - ppr->recfast_delta_z_He_1) {
        ...
        x0_new = 0.5 * (sqrt(pow((rhs - 1. - preco->fHe), 2) +
                             4. * (1. + 2. * preco->fHe) * rhs) -
                        (rhs - 1. - preco->fHe));
      ...
      /* transition finished */
        ...
        x0 = 0.5 * (sqrt(pow((rhs - 1. - preco->fHe), 2) +
                         4. * (1. + 2. * preco->fHe) * rhs) -
                    (rhs - 1. - preco->fHe));
      }
    ...
    /** - --> third approximation: first Helium recombination completed */
      ...
      if (z > ppr->recfast_z_He_2 - ppr->recfast_delta_z_He_2) {
        rhs = exp(1.5 * log(preco->CR * preco->Tnow / (1. + z)) -
                  preco->CB1_He2 / (preco->Tnow * (1. + z))) /
              preco->Nnow;
        x0_previous = 0.5 * (sqrt(pow((rhs - 1. - preco->fHe), 2) +
                                  4. * (1. + 2. * preco->fHe) * rhs) -
                             (rhs - 1. - preco->fHe));
        x0_new = 1. + preco->fHe;
        /* get s from -1 to 1 */
    ...
    /** - --> fourth approximation: second Helium recombination starts (analytic
       approximation) */
      ...
      rhs = 4. *
            ...
            preco->Nnow;
      x_He0 = 0.5 * (sqrt(pow((rhs - 1.), 2) + 4. * (1. + preco->fHe) * rhs) -
                     (rhs - 1.));
    ...
    /** - --> fifth approximation: second Helium recombination (full
      ...
      class_call(generic_integrator(thermodynamics_derivs_with_recfast, zstart,
                                    zend, y, &tpaw, ppr->tol_thermo_integration,
                                    ppr->smallest_allowed_variation, &gi),
                 gi.error_message, pth->error_message);

      y[0] = x_H0;

      /* smoothed transition */
      if (ppr->recfast_x_He0_trigger - y[1] <
          ppr->recfast_x_He0_trigger_delta) {
        rhs = 4. *
              exp(1.5 * log(preco->CR * preco->Tnow / (1. + z)) -
                  preco->CB1_He1 / (preco->Tnow * (1. + z))) /
              preco->Nnow;
        x0_previous =
            0.5 * (sqrt(pow((rhs - 1.), 2) + 4. * (1. + preco->fHe) * rhs) -
                   (rhs - 1.));
        x0_new = y[0] + preco->fHe * y[1];
        /* get s from 0 to 1 */
        s = (ppr->recfast_x_He0_trigger - y[1]) /
            ppr->recfast_x_He0_trigger_delta;
    ...
    /** - --> last case: full evolution for H and Helium */
      ...
      if (ppr->recfast_x_H0_trigger - y[0] < ppr->recfast_x_H0_trigger_delta) {
        /* get s from 0 to 1 */
        s = (ppr->recfast_x_H0_trigger - y[0]) /
            ppr->recfast_x_H0_trigger_delta;
    ...
    /* redshift */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_z) = zend;

    /* ionization fraction */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_xe) = x0;

    /* Tb */
    *(preco->recombination_table + (Nz - i - 1) * preco->re_size +
      preco->index_re_Tb) = y[2];

    /* get dTb/dz=dy[2] */
    class_call(thermodynamics_derivs_with_recfast(zend, y, dy, &tpaw,
                                                  pth->error_message),
               pth->error_message, pth->error_message);
  ...
  class_call(cleanup_generic_integrator(&gi), gi.error_message,
             pth->error_message);
 ...
 * indices) and workspace (already allocated)
 * @param error_message            Output: error message
 */

int thermodynamics_derivs_with_recfast(double z, double *y, double *dy,
  ...
  /* new in recfast 1.4: */
  double Rdown_trip, Rup_trip, tauHe_s, pHe_s, Doppler, gamma_2Ps, pb, qb,
      AHcon;
  ...
  Rdown = 1.e-19 * _a_PPB_ * pow((Tmat / 1.e4), _b_PPB_) /
  ...
  Rdown_He = _a_VF_ / (sq_0 * pow((1. + sq_0), (1. - _b_VF_)) *
                       pow((1. + sq_1), (1. + _b_VF_)));
  Rup_He =
      4. * Rdown_He * pow((preco->CR * Tmat), 1.5) * exp(-preco->CDB_He / Tmat);
  K = preco->CK / Hz;
  ...
  if (ppr->recfast_Hswitch == _TRUE_)
    K *= 1. +
         ppr->recfast_AGauss1 * exp(-pow((log(1. + z) - ppr->recfast_zGauss1) /
                                             ppr->recfast_wGauss1,
                                         2)) +
         ppr->recfast_AGauss2 * exp(-pow((log(1. + z) - ppr->recfast_zGauss2) /
                                             ppr->recfast_wGauss2,
                                         2));
  ...
  Rdown_trip = _a_trip_ / (sq_0 * pow((1. + sq_0), (1. - _b_trip_)) *
                           pow((1. + sq_1), (1. + _b_trip_)));
  Rup_trip = Rdown_trip * exp(-_h_P_ * _c_ * _L_He2St_ion_ / (_k_B_ * Tmat)) *
  ...
  if (Heflag == 0)
    ...
    if (((Heflag == 2) || (Heflag >= 5)) &&
        (x_H < 0.9999999)) { /* threshold changed by Antony Lewis in 2008 to get
                                smoother Helium */

      Doppler = 2. * _k_B_ * Tmat / (_m_H_ * _not4_ * _c_ * _c_);
      Doppler = _c_ * _L_He_2p_ * sqrt(Doppler);
      gamma_2Ps =
          3. * _A2P_s_ * preco->fHe * (1. - x_He) * _c_ * _c_ /
    ...
    if (Heflag >= 3) {
      tauHe_t = _A2P_t_ * n_He * (1. - x_He) * 3. /
                (8. * _PI_ * Hz * pow(_L_He_2Pt_, 3));
      ...
      if ((Heflag == 3) || (Heflag == 5) || (x_H >= 0.99999)) {
        ...
        gamma_2Pt =
            3. * _A2P_t_ * preco->fHe * (1. - x_He) * _c_ * _c_ /
  ...
  if (x_H > ppr->recfast_x_H0_trigger)
    ...
    if (x_H < ppr->recfast_x_H0_trigger2) {
      C = (1. + K * _Lambda_ * n * (1. - x_H)) /
          (1. / preco->fu + K * _Lambda_ * n * (1. - x_H) / preco->fu +
           K * Rup * n * (1. - x_H));
    } else {
      C = 1.;
    }
    ...
    /* coefficient as revised by Slatyer et al. 2013 (in fact it is a fit by
     * Vivian Poulin of columns 1 and 2 in Table V of Slatyer et al. 2013): */
    ...
    dy[0] = (x * x_H * n * Rdown - Rup * (1. - x_H) * exp(-preco->CL / Tmat)) *
                C / (Hz * (1. + z)) /* Peeble's equation with fudged factors */
            -
            energy_rate * chi_ion_H / n *
                (1. / _L_H_ion_ + (1. - C) / _L_H_alpha_) /
                (_h_P_ * _c_ * Hz *
                 (1. +
                  z)); /* energy injection (neglect fraction going to helium) */
  }
  ...
  if (x_He < 1.e-15)
     ...
     */
    // C_He=(1. + K_He*_Lambda_He_*n_He*(1.-x_He)*He_Boltz)/(1. +
    // K_He*(_Lambda_He_+Rup_He)*n_He*(1.-x_He)*He_Boltz);

    dy[1] =
        ((x * x_He * n * Rdown_He -
          Rup_He * (1. - x_He) * exp(-preco->CL_He / Tmat)) *
         (1. + K_He * _Lambda_He_ * n_He * (1. - x_He) * He_Boltz)) /
        (Hz * (1 + z) *
         (1. +
          K_He * (_Lambda_He_ + Rup_He) * n_He * (1. - x_He) *
              He_Boltz)); /* in case of energy injection due to DM, we neglect
                             the contribution to helium ionization */

    /* following is from recfast 1.4 */
    /* this correction is not self-consistent when there is energy injection
     * from dark matter, and leads to nan's  at small redshift (unimportant when
     * reionization takes over before that redshift) */

    if (Heflag >= 3)
      dy[1] = dy[1] + (x * x_He * n * Rdown_trip -
                       (1. - x_He) * 3. * Rup_trip *
  ...
  if (timeTh < preco->H_frac * timeH) {
     ...
     */
    dHdz = -pvecback[pba->index_bg_H_prime] / pvecback[pba->index_bg_H] /
           pba->a_today * _c_ / _Mpc_over_m_;
    ...
    dy[2] = preco->CT * pow(Trad, 4) * x / (1. + x + preco->fHe) *
                (Tmat - Trad) / (Hz * (1. + z)) +
            2. * Tmat / (1. + z) -
            2. / (3. * _k_B_) * energy_rate * chi_heat / n /
                (1. + preco->fHe + x) / (Hz * (1. + z)); /* energy injection */
  }
...
int thermodynamics_merge_reco_and_reio(struct precision *ppr,
  ...
  /** - first, a little check that the two tables match each other and can be
   * merged */

  if (pth->reio_parametrization != reio_none) {
    class_test(
        preco->recombination_table[preio->index_reco_when_reio_start *
                                       preco->re_size +
                                   preco->index_re_z] !=
            preio->reionization_table[(preio->rt_size - 1) * preio->re_size +
                                      preio->index_re_z],
        pth->error_message, "mismatch which should never happen");
  }

  /** - find number of redshift in full table = number in reco + number in reio
   * - overlap */

  pth->tt_size =
      ppr->recfast_Nz0 + preio->rt_size - preio->index_reco_when_reio_start - 1;

  /** - add  more points to start earlier in presence of interacting DM */

  if (pba->has_idm_dr == _TRUE_)
    pth->tt_size += ppr->thermo_Nz1_idm_dr + ppr->thermo_Nz2_idm_dr - 1;

  /** - allocate arrays in thermo structure */

  class_alloc(pth->z_table, pth->tt_size * sizeof(double), pth->error_message);
  class_alloc(pth->thermodynamics_table,
              pth->th_size * pth->tt_size * sizeof(double), pth->error_message);
  class_alloc(pth->d2thermodynamics_dz2_table,
              pth->th_size * pth->tt_size * sizeof(double), pth->error_message);
  ...
  for (i = 0; i < ppr->recfast_Nz0 - preio->index_reco_when_reio_start - 1;
       i++) {
    index_th = i + preio->rt_size;
    index_re = i + preio->index_reco_when_reio_start + 1;
    pth->z_table[index_th] =
        preco->recombination_table[index_re * preco->re_size +
                                   preco->index_re_z];
  ...
  if (pba->has_idm_dr == _TRUE_) {
    ...
    for (i = 0; i < ppr->thermo_Nz2_idm_dr + ppr->thermo_Nz1_idm_dr - 1; i++) {
      ...
      /* with an intermediate step Delta z =
       * (thermo_z_initial_idm_dr-recfast_z_initial)/thermo_Nz1_idm_dr/thermo_Nz1_idm_dr
       */
      if (i < ppr->thermo_Nz2_idm_dr - 1) {
        index_th = i + preio->rt_size + ppr->recfast_Nz0 -
                   preio->index_reco_when_reio_start - 1;
        pth->z_table[index_th] =
      ...
      /* with a large step Delta z  =
         (thermo_z_initial_idm_dr-recfast_z_initial)/thermo_Nz1_idm_dr */
      else {
        index_th = (i - ppr->thermo_Nz2_idm_dr + 1) + preio->rt_size +
                   ppr->recfast_Nz0 - preio->index_reco_when_reio_start - 1 +
                   ppr->thermo_Nz2_idm_dr - 1;
        pth->z_table[index_th] =
            ppr->recfast_z_initial +
      ...
      x0 = pth->thermodynamics_table[(preio->rt_size + ppr->recfast_Nz0 -
                                      preio->index_reco_when_reio_start - 2) *
                                         pth->th_size +
                                     pth->index_th_xe];
      pth->thermodynamics_table[index_th * pth->th_size + pth->index_th_xe] =
          x0;
      pth->thermodynamics_table[index_th * pth->th_size +
                                pth->index_th_dkappa] =
          (1. + pth->z_table[index_th]) * (1. + pth->z_table[index_th]) *
          pth->n_e * x0 * _sigma_ * _Mpc_over_m_;
      pth->thermodynamics_table[index_th * pth->th_size + pth->index_th_Tb] =
          pba->T_cmb * (1. + pth->z_table[index_th]);
      pth->thermodynamics_table[index_th * pth->th_size + pth->index_th_wb] =
          _k_B_ / (_c_ * _c_ * _m_H_) *
          (1. + (1. / _not4_ - 1.) * pth->YHe + x0 * (1. - pth->YHe)) *
          pba->T_cmb * (1. + pth->z_table[index_th]);
...
int thermodynamics_output_titles(struct background *pba, struct thermo *pth,
...
int thermodynamics_output_data(struct background *pba, struct thermo *pth,
                               int number_of_titles, double *data) {

  int index_z, storeidx;
  double *dataptr, *pvecthermo;
  double z, tau;

  //  pth->number_of_thermodynamics_titles =
  //  get_number_of_titles(pth->thermodynamics_titles);
  // pth->size_thermodynamics_data =
  // pth->number_of_thermodynamics_titles*pth->tt_size;
  ...
  for (index_z = 0; index_z < pth->tt_size; index_z++) {
    ...
    class_store_double(dataptr, pvecthermo[pth->index_th_dkappa], _TRUE_,
                       storeidx);
    ...
    class_store_double(dataptr, pvecthermo[pth->index_th_exp_m_kappa], _TRUE_,
                       storeidx);
    ...
    class_store_double(dataptr, pvecthermo[pth->index_th_cb2], _TRUE_,
                       storeidx);
    class_store_double(dataptr, pvecthermo[pth->index_th_tau_d], _TRUE_,
                       storeidx);
    // class_store_double(dataptr,pvecthermo[pth->index_th_rate],_TRUE_,storeidx);
    class_store_double(dataptr, pvecthermo[pth->index_th_r_d],
                       pth->compute_damping_scale, storeidx);

    if (pba->has_idm_dr == _TRUE_) {
      class_store_double(dataptr, pvecthermo[pth->index_th_dmu_idm_dr], _TRUE_,
                         storeidx);
      ...
      class_store_double(dataptr, pvecthermo[pth->index_th_tau_idm_dr], _TRUE_,
                         storeidx);
      class_store_double(dataptr, pvecthermo[pth->index_th_tau_idr], _TRUE_,
                         storeidx);
      class_store_double(dataptr, pvecthermo[pth->index_th_g_idm_dr], _TRUE_,
                         storeidx);
      class_store_double(dataptr, pvecthermo[pth->index_th_cidm_dr2], _TRUE_,
                         storeidx);
      class_store_double(dataptr, pvecthermo[pth->index_th_Tidm_dr], _TRUE_,
                         storeidx);
      class_store_double(dataptr, pvecthermo[pth->index_th_dmu_idr], _TRUE_,
                         storeidx);
    }
  }
...
int thermodynamics_tanh(double x, double center, double before, double after,
                        double width, double *result) {
